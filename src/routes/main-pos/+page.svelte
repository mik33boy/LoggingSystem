	...
	async function printReceipt() {
		try {
			...
			const thermalData = {
				...
				order_take: localStorage.getItem('order_take') || 'Dine In',
			...
			const receiptData = {
				...
				waiter_name: waiterName,
				table_number: selectedCard?.table || 'Take Out',
				order_take: selectedCard?.table || 'Take Out',
					...
					{#each [...cards, ...outdoorCards, ...gardenCards, ...takeoutCards] as card}
						<button
							...
							on:click={() => {
								...
								localStorage.removeItem('order_take'); // Delete previous order_take from local storage
								localStorage.setItem('order_take', card.order_take); // Save new order_take to local storage
								console.log('Order Take saved to local storage:', card.order_take); // Log the order_take
							}}
